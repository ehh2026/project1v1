...
- `Origin` values: `AutoSeed` (generated by script), `Manual` (user-authored), `Imported`.
...
- Seed regeneration (`generate_manual_layout_seeds.ps1`) updates the `seed-default` AutoSeed variant in each group but **never** touches Manual or Imported variants.
