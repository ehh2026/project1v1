...
- **Resetting a broken config**: A `visual-config.json` that is not valid JSON does not stop the app. It logs a warning and falls back to the **built-in code defaults** - not to `visual-config.default.json` - so the symptom is settings quietly reverting rather than a crash, and the damage is wider than the edit that caused it. To reset, delete `visual-config.json` and relaunch: the app copies a fresh one from the `visual-config.default.json` beside it. Only local tuning is lost; saved layouts, locations and content live in other files. Running `configure.bat` will detect an unreadable config, name it, and offer to move it aside for you (renaming rather than deleting, so you can still recover values by hand).
