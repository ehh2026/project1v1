...
- [x] **[URGENT] No way to unload a saved manual layout without deleting it from disk** — **code fix DONE 2026-06-24.** Added an "Unload Layout" button to the edit-mode panel (amber, non-destructive sibling of "Delete and Recalculate"). Implementation: session-scoped `LayoutEditorController.IsManualLayoutSuppressed` flag set by `UnloadManualLayout()`; the full-map auto-apply entry points (`TryApplyFullMapManualLayout`, `TryLoadFullMapManualLayoutForAnimation`, `TryApplyFullMapLayoutForZoomedSingle`) skip applying while suppressed, so pins revert to auto-placement across zoom/back/resize/tuning-reapply. The saved JSON is untouched. Suppression is cleared when a layout next becomes active (re-entering the editor reloads it from disk via `OnEditLayoutButtonClick`) and is not persisted, so an app restart restores normal auto-apply. 4 controller unit tests added (416 tests pass), build clean. **Follow-up (minor):** multi-location cluster-key layouts applied via `_savedLayoutToApply` are not yet covered by the suppression flag (only full-map layouts are); add the same guard there if cluster-key unload is needed. **Pending: GUI confirmation.**
