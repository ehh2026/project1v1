...
> Historical note: this completed plan predates [remove-pins-jpg-legacy-path-plan.md](remove-pins-jpg-legacy-path-plan.md). Any instructions below that mention keeping an `ImagePinMarker` wrapper are superseded; current edit/drag handling stays attached to the `LocationMarker` canvas item, with drawn `PinMarker` fallback or `CompositePinMarker` content only.
...
**Current state:** `MainWindow.xaml.cs` is ~**2000 lines** (repo rule: keep `.cs` files under **800** — see [ARCHITECTURE.md](../../../ARCHITECTURE.md), [REFACTORING_ASSESSMENT.md](../../assessments/REFACTORING_ASSESSMENT.md)). The naive reading of this plan adds Reassign, exit-replay, overrides, cache wiring, and menu construction to MainWindow — that would make the god-object problem worse.
