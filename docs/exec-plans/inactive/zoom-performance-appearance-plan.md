...
started: 2026-06-23
parked: 2026-09-08
parked_reason: User-directed hold pending reprioritization of zoom work
...
- [ ] **2.6 High-quality settled full-map rendering + render-options cleanup** (`Views/MapDisplayControl.xaml(.cs)`) — **code complete; automated verification passed (552 tests); live visual pending after Windows app-control approval timed out July 1, 2026.** The implementation keeps `Stretch="Fill"` and pixel snapping, replaces local `NearestNeighbor` with settled `Fant`, removes `EdgeMode="Aliased"`, and sets the unchanged cache hint once rather than on every viewport update. Follow [the approved design](../../superpowers/specs/2026-07-01-map-render-quality-design.md) and [implementation plan](../../superpowers/plans/2026-07-01-map-render-quality.md). **Remaining verification:** inspect the `U` in `SOUTH AMERICA` and representative thin grid/coastline strokes; confirm marker/input alignment and full-screen Fill geometry.

### 2c. Appearance polish

- [ ] **2.7 Explicit animation and settled-zoom scaling policy** — **code complete; automated verification passed; live zoom-in/out visual pending after the same app-control timeout.** Keyframes select `Linear` before bitmap materialization, `AnimationFrameCache.CacheVersion` is 16 so old pixels cannot bypass the change, and the existing full-resolution `Fant` path remains in `ZoomedRegionCache`. Detailed steps and the output-size visual matrix are in [2026-07-01-map-render-quality.md](../../superpowers/plans/2026-07-01-map-render-quality.md). This resolves **Decision D3** without adding the separately deferred settled full-map render cache.
  - **Settled-zoom follow-up code complete; automated gate components pass (579 tests):** [2026-07-01-zoomed-map-upscaling.md](../../superpowers/plans/2026-07-01-zoomed-map-upscaling.md) now makes output DPI/physical-pixel correct, uses cache schema 8 with source/render identity, and adds Runtime Tuning comparisons for `Fant`, `Lanczos3`, `MitchellNetravali`, `Bicubic`, and restrained sharpened bicubic. Animation remains `Linear`; `Fant` remains the default pending live comparison. Generated 1440p custom modes take about 1.7-3.3 seconds on first generation and are disk-cached afterward.
