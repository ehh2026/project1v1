...
| A cluster (zoomed in) | `{hash}_z{zoom}_c{cx}_{cy}_s{W}x{H}_m{…}_p{…}_l{…}_n{…}` | `LayoutKeyGenerator.GenerateKey()` |
...
The full-map key is deliberately size-independent, so resizing the window cannot orphan it. Cluster
keys are not — see Trap 1.
