...
- **Drawn-pin divot cap (opt-in):** `PinMarkers.DrawnPinTipCap` now renders `Horizontal` and `Concave` as open, unfilled, near-black strokes. Concave bows away from the pin head, keeps its true midpoint at the shaft tip, and flips when a manual layout places the head below the tip. Width, line weight, and curvature are configurable and available in Runtime Tuning; `Style` remains default-off.
...
`PinMarkers.DrawnPinTipCap` applies at the visible terminus of either a built-in stub or extension line. Caps remain horizontal in screen space, render above extension lines and below heads, and track hover, drag, and zoom. Normal/inverted visual sign-off and the remaining interaction smoke matrix are still pending. (`docs/exec-plans/active/drawn-pin-tip-cap-plan.md`)
