...
- **A crash now leaves evidence, and usually is not a crash:** unhandled errors were ending the application with nothing written down — the window simply closed and `app.log` stopped mid-action, which is unhelpful when the report comes from a machine you cannot inspect. Every unhandled error is now logged, and one that reaches the UI no longer closes the app: the map stays on screen. An error that does end the process is written straight to `app.crash.log` first, because the ordinary log writer is a background thread that shutdown abandons. If a zoom animation fails part-way it now lands on the view it was heading for instead of freezing. What a gallery machine would still need to run unwatched is written down in [docs/reference/unattended-kiosk.md](docs/reference/unattended-kiosk.md).

- **The map restarts itself when it closes:** `Tools\Run-Unattended.bat` starts the map and starts it again whenever it exits, so a crash or an accidental quit recovers without anybody present. A shortcut to it in the Startup folder brings the map up after a reboot. It needs no administrator rights and installs nothing — removing it is deleting the shortcut. It cannot detect a hang, since a frozen app has not exited. Setup steps ship with the release; the wider gaps in running unattended are in [docs/reference/unattended-kiosk.md](docs/reference/unattended-kiosk.md).
...
- **Trap 1, said accurately (Phases 6.5-6.6):** editing a `RadialExtension` value in `visual-config.json` changes every cluster key, but the four settings do not cost the same. `ProximityThresholdPixels` and `MinLocationsForExtension` decide which locations form a cluster, so changing either *far enough to move a location in or out* makes the cluster a different set, which hashes differently — and the hash is one of the two things `AreKeysCompatible` compares. Saved layouts for those clusters stop resolving; raising the minimum past a cluster's size removes the cluster altogether. A change that leaves every cluster's membership as it was costs nothing: the key string moves but the hash does not, so the layout still loads. That makes the grouping pair conditional rather than automatic — which is not a reassurance, since whether a given edit crosses that line depends on the gaps between locations and is not visible from the config file, so the warning treats the pair as dangerous throughout. `ExtensionLineLength` and `MinimumLineLength` only affect how a line is drawn, so the layout still loads; what remains there is that later saves land in a new group and auto-placed pins are recalculated around the manual-layout ones. `configure.ps1` now warns about both, separately, so the destructive pair is not buried in the mild one. New tests cover full-map versus cluster keys never colliding or resolving to each other, two distinct clusters likewise, and both halves of the config-change behaviour measured end to end — the grouping half driven through `DetectDenseGroups`, since a fixed location list cannot show it.

- **Window size is out of cluster layout keys (Phase 6.9):** `s{W}x{H}` was baked into every cluster key but never consulted by `AreKeysCompatible`, so it split one cluster into a separate group per window size while changing nothing about which layout was found — the shipped file holds the same clusters four times over, one per size someone happened to run at. Full-map keys dropped it long ago for the same reason. Keys written before this still resolve, since compatibility only ever read the hash and the zoom, and `MergeSizedClusterGroups` collapses groups still keyed that way the next time the file is read or written, so the duplication goes away instead of being tolerated. Where merged groups hold a variant with the same id — each sized group has its own `manual-default` — manual layouts are never dropped: the second keeps its layout under a suffixed id and a name saying which size it came from, because an awkward name is recoverable and a missing layout is not. A generated seed never displaces a manual or imported variant that happens to share its id either — the seed is discarded instead, since regenerating it costs a rerun of the generator while the variant is whatever someone saved as a manual layout. Two seeds collapse to the newer one. Saving still writes under the current view's key without compatible-key resolution: reading resolves because a layout that exists should be found, but collapsing compatible groups on save would decide where layouts are stored rather than where they are found.

- **A layout key now means one thing to every part of the editor (Phase 6.8):** `LoadLayout` fell back to a compatible key while `ListVariants`, `LoadVariant`, `DeleteVariant` and `DeleteLayout` matched the session key exactly. Outside the seeded window sizes — a different monitor, a docked laptop — the map showed a saved layout the dropdown reported as absent. Fixing only the listing would have been worse: variants shown that selecting silently failed to load, and that "Delete This Layout" reported removing without removing. All of them now resolve through one place, `ResolveGroupKey` — as do `SetSelectedVariantId`, `SetDefaultVariant` and `GetSelectedVariantId`, which otherwise wrote a chosen variant under a key nothing reads back, so a selection appeared to save and was gone on the next load. The fallback is no wider than `AreKeysCompatible`, so a different cluster is still a different layout, and an exact match still beats a compatible one. **Saving is deliberately not included:** a save still writes under the current view's key, which is the fragmentation Phase 6.9 addresses. The resolution helpers moved to `ManualLayoutManager.Resolution.cs` — they answer one question that everything group-scoped depends on agreeing about.

- **Saved layouts are told apart in the picker (Phase 6.7):** every generated seed on disk is stored as `Generated Seed`, so each view's dropdown showed one identical row and looked like the same layout everywhere. The picker now shows a label built when the list is read — name, pin count, and whether it is the default — so what is already on disk is distinguishable without rewriting it. Newly generated seeds are named after their cluster (`Seed: New York, Newark, +2 more`), which also makes the raw JSON readable by hand. The view being edited is named once above the picker rather than repeated on every row, since the panel is a fixed-width overlay and every row belongs to that view anyway.

- **Unload survives re-opening the editor:** entering Edit Layout treated a suppressed layout as one becoming active again and re-applied it, so "Unload and Recalculate" was undone by the next click on the button next to it. Opening a view to work on it is not a request to undo the unload; saving is, and that clears the suppression by itself. The editor still loads the layout on entry to adopt its variant identity — otherwise saving over an unloaded layout would write a second variant beside it rather than replacing it — it just no longer puts the positions back on screen.

- **Hover feedback on the panel buttons now actually happens:** `BackButton`, `EditLayoutButton`, `SaveLayoutButton`, `UnloadLayoutButton`, `DeleteLayoutButton`, `ExitEditModeButton` and the content window's `TranslateButton` each declared an `IsMouseOver` trigger that changed `Background`, while also setting `Background` directly on the element. A value set on an element is a *local value* and outranks a style trigger in WPF's precedence order, so every one of those triggers was dead — silently, with no error or warning, since the markup is perfectly valid. `Background` moved into each `Style` as a Setter. `Tests/XamlLocalValueTests` now scans every XAML file for the pattern, so the next control written this way fails the build rather than quietly not working.

- **"Auto Assign Pins" is disabled when the map is drawing its pins:** the button re-picks an image shaft and head for each pin, which only means anything with composite pin rendering on. It was not merely useless in drawn-pin mode — nothing on the apply path checks the rendering mode, so a click replaced drawn pins with composite ones for the markers it touched, leaving the map in a state `visual-config.json` does not describe. It is now greyed out — including visually. That needed two things: an `IsEnabled="False"` trigger, because the button's custom control template replaces WPF's default chrome along with the greying it applies; and moving `Background`, `Foreground`, `BorderBrush` and `Cursor` off the `Button` element into the `Style`, because a value set directly on an element is a *local value*, which outranks a style trigger in WPF's precedence order. A trigger that sets one is ignored silently — no error, no warning — so the first attempt changed the cursor and tooltip while leaving the button its original colour. There is a tooltip saying why it is disabled (kept visible on the disabled button, which WPF otherwise suppresses), and the handler refuses to run regardless of button state. Availability is set when the edit panel opens, and again after a tuning apply as redundancy: tuning currently cannot be applied during edit mode at all, so the button's correctness should not rest on that guard staying in place.

- **The edit panel now says which view you are editing (Phase 6.1–6.4):** manual layouts are per view and always have been, but the panel looked identical whichever view was open, so there was no way to tell what a save was about to write to — and no way to tell two clusters apart. It now shows `Editing: Whole map` or `Editing: Cluster: New York, Newark, +2 more`, taken from the edit session itself rather than rebuilt in the panel, so the label cannot name a scope the save will not reach. The variant dropdown is labelled `Saved layouts for this view:` and, when empty, distinguishes the two reasons it can be: `None saved for this view yet` only when nothing is loaded either, and `Layout loaded, but not listed at this window size` when a layout is applied that the picker cannot find (Trap 3 — loading falls back to a compatible key, the picker matches exactly). Claiming nothing is saved while the map shows a saved layout would be a confident denial of work the user is looking at, and the obvious response to it is to redo the layout. Previously an empty dropdown over a blank status line was the "my layouts are gone" report, when the layouts were filed under a view the user was not looking at. The five actions are regrouped — Save and Save As, then a separator captioned `Stop using this layout`, then Unload, Delete This Layout, Delete ALL — because previously the two saves were separated by a delete and the column read as five similar options. New reference doc [docs/reference/manual-layout-scoping.md](docs/reference/manual-layout-scoping.md) covers both key shapes, the three traps (config edits orphaning cluster layouts, compatibility being looser than the key, and `ListVariants` reporting no variants for a layout that is actively applied), which file the app actually reads, and where the identical-looking `Generated Seed` entries come from. Linked from `docs/index.md` and `CLAUDE.md`.

- **Extension-line heads no longer leave the canvas (Phase 5):** `RadialExtensionCalculator.CalculateMaxLength` worked out how much room was left before the edge and then raised the result back to a 20px minimum, so a marker 5px from the top still put its head at Y≈-6.09 on a 100×100 canvas — off-canvas, where it is not drawn at all. A minimum length in a bounds clamp is a minimum distance past the edge, so both floors (the one inside `CalculateMaxLength` and a second `< 20` clamp at the call site) are gone: near an edge the line is now simply short. One clamp is kept, for the marker already outside the canvas: the intermediate distance-to-edge candidate comes out negative there, which would otherwise point the line backwards through its own marker, so `CoordinateMapper.DistanceToCanvasEdge` returns 0 instead. It never returns a negative distance. The same floor existed a second time downstream: `RadialExtensionAdjuster` runs after the calculator, separates overlapping heads by changing line lengths — it can lengthen as well as shorten — and applies its own `MinimumLineLength`, which near an edge is a minimum distance past the edge. It has no canvas to check against, so `MarkerPlacementOrchestrator` now re-clamps after adjustment, where the container size is known; separation loses to bounds when they disagree, since two heads slightly too close are both still visible and one pushed off the edge is not. Without that clamp a head in the new pipeline test lands at Y=-55.95. Where the canvas edge is now has a single definition, `CoordinateMapper.DistanceToCanvasEdge`, so the calculator and the post-adjustment clamp cannot disagree about it. This un-skips `CalculateExtensions_WithCanvasBounds_KeepsHeadsInsideBounds`, leaving one skipped test in the suite (a testability seam, not a product bug).

- **Layout editor delete actions (Phase 2):** "Delete and Recalculate" destroyed every manual layout saved for the view, with no confirmation of any kind, from behind the mildest-sounding label in the panel. The three actions now say what they destroy and ask before doing it. **Unload and Recalculate** (was "Unload Layout") reverts to automatic placement and destroys nothing; it is now the panel’s most prominent action, since it is what reaching for the red button usually meant. **Delete This Layout** (was "Delete Variant") deletes only the variant on screen, naming it in the prompt. **Delete ALL Manual Layouts** keeps the bulk behaviour but lists every variant it will destroy and states the count; generated seeds are untouched, so the count reflects only manual-layout work. Both prompts default to No, so a reflexive Enter cancels. With nothing to delete, the bulk button says so and points at Unload rather than acting. User-facing summary in [docs/reference/layout-editor-known-issues.md](docs/reference/layout-editor-known-issues.md).

- **Layout edit-session context, Phase C (PR #15):** The ambient layout key is deleted, not guarded. `CurrentLayoutKey` and `SetLayoutKey` no longer exist, so the field that navigation and the editor both wrote — the root cause behind every layout-data-loss fix in PRs #13 and #14 — cannot be pointed at the wrong scope, because there is nothing left to point. Display and replay now carry their layout key explicitly through loading, zoom, and application instead of reading it back from shared state, and loading is side-effect free: opening a layout for display no longer adopts its variant identity, which happens once on edit entry. **Behaviour change:** the collapse guard’s drag-based exception is gone. Previously, dragging every pin in a dense cluster back onto its own location let a save through; now such a save is always refused as collapsed. That case (smoke S10) was judged not a real workflow, and the exception inferred intent from coordinates — the same kind of reasoning that produced the original data loss. Sparse views are unaffected and still save normally (smoke S8). Completes the migration begun in the PR #14 entry below, which no longer needs its "display and replay still use the previous key handling" caveat.

- **Root-level configuration entry points:** Added `toggle-dev-tools.bat`, `configure.ps1` and its `configure.bat` wrapper at the project root. The `.bat` wrapper exists because cmd.exe and Explorer cannot execute a `.ps1` — running `scripts\toggle-dev-tools.ps1` from a cmd window made Windows ask "How do you want to open this file?" and silently changed nothing, so developer tools stayed off. It forwards all arguments (`-State on|off|toggle`, `-PublishDir`) and propagates the script’s exit code, so a failure such as "no built exe found" is visible to the caller instead of being swallowed. Both end with a "Press Enter to close" when run interactively, so a double-click from Explorer does not close the window before the output can be read — which for `configure.ps1` would mean losing the entire point of running it. Detecting a double-click specifically is not reliable (`explorer.exe` is an ancestor of an ordinary console session too, and a double-click’s command line is near-identical to what PowerShell produces when running the wrapper from a prompt), so the pause is unconditional and suppressed explicitly instead: `-NoPrompt` for `configure.ps1`, and a new `-NoPause` for `toggle-dev-tools.ps1`. `configure.ps1` passes `-NoPause` when it invokes the toggle, and now also surfaces and propagates that script’s exit code, so a toggle that failed because no built exe exists is no longer reported as success. `configure.ps1` also explains how to recover a broken config: an unparseable `visual-config.json` does not stop the app, it falls back to the built-in code defaults — not to `visual-config.default.json` — so the symptom is settings quietly reverting rather than a crash. The script names any config it cannot parse and offers to move it aside (renaming, not deleting) so the app re-creates it from the shipped defaults on the next launch. Both scripts also trap terminating errors so a failure prints and pauses instead of closing the window on the message. `configure.ps1` is read-only: it prints each config file (`locations.json`, `manual-layouts.json`, `visual-config.default.json`, and the runtime `visual-config.json` next to every built exe) with its resolved absolute path, whether it exists, and the current developer-tools state, then offers to run the toggle through the `.bat`. `docs/guides/BUILDING_EXECUTABLE.md` no longer claims the toggle script cannot reach publish output — it has scanned every exe folder since the `-PublishDir` work.

- **Layout edit-session context, Phases A+B (PR #14):** Edit scope is now captured once, immutably, when the editor is opened — the layout key, whether it is the whole map or a cluster, and the viewport it was derived against — and every save, delete and variant operation reads from that capture instead of a field navigation also writes. **Behaviour change:** opening the editor now loads the session's layout unconditionally in order to adopt its variant identity, so a subsequent Save updates the variant that was actually loaded rather than targeting `manual-default`. That identity previously arrived as a side effect of navigation's probe loads, which have been made side-effect free. Resize-staleness is now derived by comparing the live view against the session's captured viewport rather than tracked by a flag, so the `✗ SAVE ABORTED — VIEW CHANGED` refusal no longer depends on a caller remembering to set one. Variant identity cannot leak between scopes, since scope only changes by beginning a session. Supersedes the guarded implementation described in the PR #13 entry below for the edit paths; display and replay still use the previous key handling.

- **Layout editor data-loss fixes (PR #13):** The editor now derives its layout key from the view on screen instead of inheriting a stale one, so a cluster edit can no longer overwrite the full-map layout. Every save path — Save and Save As — goes through one guarded capture that verifies scope and marker geometry before writing, and refuses rather than persisting guesses. Saved cluster layouts no longer replay as short vertical stubs: a head offset authored zoomed in was re-projected at the full-map scale, collapsing ~59px to 0.46px, under the 5px extension threshold. Layout precedence is now by origin rather than scope (Manual zoomed → Manual full-map → AutoSeed), so a manual zoomed arrangement is displayed instead of being saved and ignored. Resizing the window mid-edit no longer re-places markers, and marks the session's geometry stale so a later save is refused instead of mixing pre-resize endpoints with new anchors. Each save keeps a rolling `.bak` of the layout file. User-facing summary in [docs/reference/layout-editor-known-issues.md](docs/reference/layout-editor-known-issues.md).
...
- **Hitbox drift & large-image robustness:** Saved manual layouts now re-sync pin hit targets after the layout fully settles (depth sort/tip caps), matching the Delete & Recalculate path, so pin heads in dense zoomed clusters are no longer off-target or unclickable. Content images are decoded within the display's physical pixel box (bounded decode with all image IO on a background thread) and never left unbounded (config default 3840×2160 with a 4K floor), so very large TIFFs no longer hang the content popup — it stays closeable/switchable while loading. Heavy files (default ≥75 MB) and any downscale surface an on-screen notice and actionable log warning, gated behind new `Debug.LogContentImageDiagnostics` (dev-tools only); downscaling itself always runs. Added `scripts/toggle-dev-tools.ps1` to flip `EnableDeveloperTools` in the runtime config without hand-editing JSON; it now updates every `visual-config.json` next to a built or **published** exe (Debug/Release/`publish/`/self-contained) and accepts `-PublishDir` for an external publish output. The decode box is sized from the **window's own monitor** (its actual size) rather than the primary screen, so a kiosk on a secondary display is bounded correctly.
- **Follow-through review hardening:** Excel workbook/rels DOM loads now use the same prohibited-DTD `XmlReaderSettings` as streaming parse; lowering `MaxCachedLocations` evicts immediately; default verify/CI exclude `Category=Performance` cluster timing tests; CHANGELOG archive links retargeted to `completed/`.
...
- **Refactoring follow-through plan decisions:** Locked Phase 11–17 design choices in [refactoring-assessment-followthrough-plan.md](docs/exec-plans/completed/refactoring-assessment-followthrough-plan.md) (display-space SoT + validator ceiling fix; delete `ApplicationState`; spatial grid 3×3; XmlReader Excel sketch; narrowed ContentLoader LRU). Execution order aligned; assessment §6 logging and gallery multi-image cache explicitly deferred.
- **Refactoring follow-through plan refresh:** Updated [refactoring-assessment-followthrough-plan.md](docs/exec-plans/completed/refactoring-assessment-followthrough-plan.md) for current code (Phase 12 obsolete, Phase 13 and Phase 18 large-file slice done). Remaining work ordered for implementation (map metadata → ApplicationState → spatial clusterer → Excel streaming → ContentLoader LRU). Living TO_DO / tech-debt entries updated; historical assessments and completed plans left unchanged.
- **Stale active-plan taste policy:** Incomplete `docs/exec-plans/active/` plans older than 30 days now produce non-blocking warnings in `verify_taste.py` and `doc_gardening.py` (and therefore pre-push/verify/CI taste steps) instead of failing the harness. Warning text tells agents to report the issue to the user and ask whether to continue, park (`inactive/`), or archive (`completed/`).
- **Complexity and CI gates:** Added repo-wide formatting/analyzer settings, `.runsettings` coverage exclusions, blocking CI/local coverage thresholds, Lizard complexity gates, a formatting pre-commit hook, and a build-aware pre-push hook. Refactored the 10 planned high-complexity methods below CCN 15 and added focused parser, Excel error-path, and coverage-tool tests.
...
- **Exec plans:** [manual-layout-seed-alignment-plan.md](docs/exec-plans/active/manual-layout-seed-alignment-plan.md), [manual-layout-variants-plan.md](docs/exec-plans/completed/manual-layout-variants-plan.md), [composite-pins-unzoomed-plan.md](docs/exec-plans/active/composite-pins-unzoomed-plan.md), [refactoring-assessment-followthrough-plan.md](docs/exec-plans/completed/refactoring-assessment-followthrough-plan.md) — linked from [docs/TO_DO.md](docs/TO_DO.md)
