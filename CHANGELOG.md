...
- **"Auto Assign Pins" is disabled when the map is drawing its pins:** the button re-picks an image shaft/head pair for each marker, which only means anything with composite pin rendering on. It was not merely useless in drawn-pin mode — nothing on the apply path checks the rendering mode, so a click replaced drawn pins with composite ones for the markers it touched, leaving the map in a state `visual-config.json` does not describe. It is now greyed out, with a tooltip saying why (kept visible on the disabled button, which WPF otherwise suppresses), and the handler refuses to run regardless of button state. Availability is re-evaluated whenever the tuning panel changes the composite toggle, since that can happen without leaving edit mode.
