...
- **"Auto Assign Pins" is disabled when the map is drawing its pins:** the button re-picks an image shaft and head for each pin, which only means anything with composite pin rendering on. It was not merely useless in drawn-pin mode — nothing on the apply path checks the rendering mode, so a click replaced drawn pins with composite ones for the markers it touched, leaving the map in a state `visual-config.json` does not describe. It is now greyed out — including visually, which its custom control template had dropped along with WPF's default disabled chrome, and with the hand cursor moved into the style so a disabled button stops inviting the click — with a tooltip saying why (kept visible on the disabled button, which WPF otherwise suppresses), and the handler refuses to run regardless of button state. Availability is set when the edit panel opens, and again after a tuning apply as redundancy: tuning currently cannot be applied during edit mode at all, so the button's correctness should not rest on that guard staying in place.
