...
- **"Trap 1" was not true (Phases 6.5-6.6):** the plan, the scoping doc and the open-questions list all said that editing a `RadialExtension` value in `visual-config.json` orphans every saved cluster layout — that they stay on disk but stop resolving, presenting as layouts vanishing. They do not, and never did. Those values are part of the cluster key, but `AreKeysCompatible` has only ever compared the location hash and the zoom, so a group saved under the old settings stays compatible with the new key and still loads. Measured through `ManualLayoutManager` in `ConfigChangeDoesNotHideSavedLayoutsTests`, rather than argued from the key format, which is how the wrong version survived being repeated across three documents. The vanishing reports were Trap 3 — `ListVariants` matching the key exactly, so the dropdown emptied while the map went on showing the layout — fixed in Phase 6.8. `configure.ps1` now warns about what does happen: saving keys exactly, so anything saved after the change lands in a new group beside the old one, and pins auto-placed around the hand-placed ones are recalculated with the new numbers. New key-property tests cover full-map versus cluster keys never colliding or resolving to each other, two distinct clusters likewise, and the config-change behaviour as it actually is.

- **Window size is out of cluster layout keys (Phase 6.9):** `s{W}x{H}` was baked into every cluster key but never consulted by `AreKeysCompatible`, so it split one cluster into a separate group per window size while changing nothing about which layout was found — the shipped file holds the same clusters four times over, one per size someone happened to run at. Full-map keys dropped it long ago for the same reason. Keys written before this still resolve, since compatibility only ever read the hash and the zoom, and `MergeSizedClusterGroups` collapses groups still keyed that way the next time the file is read or written, so the duplication goes away instead of being tolerated. Where merged groups hold a variant with the same id — each sized group has its own `manual-default` — hand-made work is never dropped: the second keeps its layout under a suffixed id and a name saying which size it came from, because an awkward name is recoverable and a missing layout is not. A generated seed never displaces a hand-made or imported variant that happens to share its id either — the seed is discarded instead, since regenerating it costs a rerun of the generator while the variant is whatever someone laid out by hand. Two seeds collapse to the newer one. Saving still writes under the current view's key without compatible-key resolution: reading resolves because a layout that exists should be found, but collapsing compatible groups on save would decide where layouts are stored rather than where they are found.
...
- **Layout editor delete actions (Phase 2):** "Delete and Recalculate" destroyed every hand-made layout saved for the view, with no confirmation of any kind, from behind the mildest-sounding label in the panel. The three actions now say what they destroy and ask before doing it. **Unload and Recalculate** (was "Unload Layout") reverts to automatic placement and destroys nothing; it is now the panel’s most prominent action, since it is what reaching for the red button usually meant. **Delete This Layout** (was "Delete Variant") deletes only the variant on screen, naming it in the prompt. **Delete ALL Saved Layouts** keeps the bulk behaviour but lists every variant it will destroy and states the count; generated seeds are untouched, so the count reflects only hand-made work. Both prompts default to No, so a reflexive Enter cancels. With nothing to delete, the bulk button says so and points at Unload rather than acting. User-facing summary in [docs/reference/layout-editor-known-issues.md](docs/reference/layout-editor-known-issues.md).
