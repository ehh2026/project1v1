...
- **Workbook-backed content tables:** `Coordinates for map.xlsx` now includes per-location image filename columns plus new `Bio Text` and `Captions` sheets. `ContentLoader` and `ExcelCoordinateReader` now prefer the workbook-backed image order, didactic text, and captions while preserving legacy `.txt` sidecar fallback.
