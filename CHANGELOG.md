...
- **Content folder reorganization:** Moved location subfolders, Excel coordinates, JSON configurations, and static assets from a flat layout under `Images&Content/` to structured `Assets/`, `Demo-Content/`, and `Extras/` directories. Updated `ContentLoader` and `StartupValidator` to support loading from `Production-Content/` or `Demo-Content/` with legacy fallbacks, namespaced AppData layouts/clusters by active set suffix, and fixed relative path resolution for pin-part geometries.
