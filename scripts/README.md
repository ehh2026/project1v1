...
| `prepare_web_assets.py` | Manual (web pre-bake, see [web-map-plan.md](../docs/exec-plans/active/web-map-plan.md)) | venv (Pillow) | Builds the static `web/` bundle: intermediate base map from the 181 MP master, deterministic tile pyramid (`--tiles-native` adds the ~16384 px native level), lazy/device-safe regional crops (per-crop decoded-pixel + byte budgets with deterministic splitting), bounded popup-image derivatives, `web/data/locations.json` with stable slug ids, normalized coords + pre-baked altText. Run with the repo venv python |
| `verify_web_bundle.py` | Manual (after pre-bake; also wired into `prepare_web_assets_tests`) | stdlib | Coherence gate over the generated `web/` bundle: unique slug ids, `[0,1]` coords, allow-list image/crop paths (no `.`/`..` traversal), sample tile grid per level, tile `baseZoom`/`tileSize`/numeric levels, and the crop-payload budget. `py -3 -m unittest scripts.prepare_web_assets_tests` runs it whenever a generated bundle is present (skips when `web/data/locations.json` is absent — it is gitignored) |
| `prepare_web_assets_tests.py` | Manual | stdlib / venv | Regression tests for the pre-bake: coordinate validation/fallback, derivative name sanitization, crop budget + splitting, tile geometry/top-padding, tile-vs-master MSE alignment, stable-id reorder-invariance, and the bundle coherence gate |
